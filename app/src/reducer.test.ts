...
describe("reducer", () => {
  ...
  it("adds a task with normal priority by default", () => {
    const next = reducer(initialState, addTask("a", "A"));
  ...
  it("does not mutate state when setting priority (immutability)", () => {
    const withTask = reducer(initialState, addTask("a", "A"));
    ...
    expect(next.tasks[0]).not.toBe(withTask.tasks[0]);
  });

  it("leaves other tasks untouched for an unknown id priority change", () => {
    const withTask = reducer(initialState, addTask("a", "A"));
