...
5. Do NOT introduce a state-management library or a new npm dependency as
   part of a "cleanup."
6. Run `cd app && npm test` and `npm run typecheck` — existing tests must
   still pass unchanged (if a test needs to change, that's a sign behavior
   changed — stop and flag it instead of editing the test to match).
